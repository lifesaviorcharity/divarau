...
import { Search, Eye, CheckCircle, XCircle, Trash2, Edit, PowerOff } from "lucide-react";
...
export default function AdsClient({ initialAds }: { initialAds: any[] }) {
  ...
  const handleReject = async (type: "reject" | "needs_edit" | "disable", id?: number) => {
    ...
    const finalNote = type === "needs_edit" ? `[NEEDS_EDIT] ${adminNote}` : (type === "disable" ? "[DISABLED]" : adminNote);
    try {
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {(ad.status === "FINAL" || ad.status === "APPROVED" || ad.status === "PAID") && (
                        <button onClick={() => { if(confirm("آیا از غیرفعال کردن این آگهی اطمینان دارید؟")) handleReject("disable", ad.id); }}
                          className="p-1.5 text-gray-400 hover:text-gray-800 hover:bg-gray-200 rounded-lg" title="غیرفعال کردن">
                          <PowerOff size={14} />
                        </button>
                      )}
