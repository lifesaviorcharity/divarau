        ...
        <div className="divide-y divide-gray-100 overflow-y-auto">
          {categories.map((cat) => (
            <div key={cat.id} className="flex flex-col">
              ...
              {/* SubCategories List */}
              {expandedCats[cat.id] && (
                <div className="bg-slate-50/70 border-t border-gray-300 px-4 sm:px-10 py-3.5 transition-all">
                  {cat.subCategories?.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-6 text-xs text-gray-400 gap-2">
                      <span>هیچ زیردسته‌ای برای این گروه ثبت نشده است.</span>
                      <button
                        onClick={() => setSubModal({ isOpen: true, mode: "add", data: null, catId: cat.id })}
                        className="text-primary font-semibold hover:underline flex items-center gap-1 text-xs"
                      >
                        <Plus size={14} /> افزودن اولین زیردسته
                      </button>
                    </div>
                  ) : (
                    <div className="pr-3 sm:pr-4 border-r-2 border-primary/20 space-y-1 my-1">
                      {cat.subCategories?.map((sub: any) => (
                        <div
                          key={sub.id}
                          className="flex items-center justify-between group py-2.5 px-3.5 hover:bg-white rounded-xl transition-all gap-3 hover:shadow-sm"
                        >
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <span className="w-1.5 h-1.5 rounded-full bg-primary/30 group-hover:bg-primary transition-colors shrink-0" />
                            <span className="text-sm font-semibold text-gray-800 truncate">{sub.name}</span>
                            <span className="hidden sm:inline-block text-[10px] text-gray-400 font-mono bg-gray-100/80 px-2 py-0.5 rounded-md truncate dir-ltr">
                              {sub.slug}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            {/* Counts badges */}
                            <div className="hidden md:flex items-center gap-1.5 text-[11px] font-medium text-gray-500">
                              <span className="px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 border border-blue-100/60">
                                {sub._count?.jobs || 0} شغل
                              </span>
                              <span className="px-2 py-0.5 rounded-md bg-amber-50 text-amber-700 border border-amber-100/60">
                                {sub._count?.ads || 0} آگهی
                              </span>
                            </div>
