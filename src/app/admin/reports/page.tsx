...
export default function AdminReportsPage() {
  ...
  const maxJobs = data?.chartData ? Math.max(1, ...data.chartData.map((d) => d.jobs)) : 1;
  const maxAds = data?.chartData ? Math.max(1, ...data.chartData.map((d) => d.ads)) : 1;
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="space-y-3 min-h-[150px]">
            {loading ? (
              ...
              <div key={d.label} className="flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col justify-center gap-1 py-1 border-r-2 border-gray-100 pr-2">
                  <div className="h-4 bg-primary/20 rounded-l-lg transition-all duration-500 relative"
                    style={{ width: `${Math.max(2, (d.jobs / maxJobs) * 95)}%` }}>
                    <div className="absolute inset-y-0 right-0 w-full bg-primary rounded-l-lg flex items-center justify-end px-1.5 overflow-hidden">
                      {d.jobs > 0 && <span className="text-[9px] text-white font-bold">{d.jobs}</span>}
                    </div>
                  </div>
                  <div className="h-4 bg-blue-100 rounded-l-lg transition-all duration-500 relative"
                    style={{ width: `${Math.max(2, (d.ads / maxAds) * 95)}%` }}>
                    <div className="absolute inset-y-0 right-0 w-full bg-blue-500 rounded-l-lg flex items-center justify-end px-1.5 overflow-hidden">
          ...
          <div className="flex items-center gap-4 mt-4 pt-3 border-t border-gray-100">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-gray-500">مشاغل</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded bg-blue-500" />
              <span className="text-[10px] text-gray-500">آگهی‌ها</span>
            </div>
          </div>
        </div>
